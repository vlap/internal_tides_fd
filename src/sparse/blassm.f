...
c-----------------------------------------------------------------------
      subroutine diamua_cmplx (nrow,job, a, ja, ia, diag, b, jb, ib)
      use precisions, only: wp, cwp
      real(wp) diag(nrow), scal
      complex(cwp) a(*), b(*)
      integer ja(*),jb(*), ia(nrow+1),ib(nrow+1)
c-----------------------------------------------------------------------
c performs the matrix by matrix product B = Diag * A  (in place)
...
c-----------------------------------------------------------------------
      ...
      subroutine dia_int_mua_cmplx(nrow,job, a, ja, ia, diag, b, jb,ib)
      use precisions, only: wp, cwp
      integer diag(nrow), scal
      complex(cwp) a(*), b(*)
      integer ja(*),jb(*), ia(nrow+1),ib(nrow+1)
c-----------------------------------------------------------------------
c performs the matrix by matrix product B = Diag * A  (in place)
...
c-----------------------------------------------------------------------
      subroutine aplsca_cmplx (nrow, nz, a, ja, ia, scal,iw)

      use precisions, only: cwp

      complex(cwp) a(nz), scal
      integer ja(nz), ia(nrow+1),iw(nz)
c-----------------------------------------------------------------------
c Adds a scalar to the diagonal entries of a sparse matrix A :=A + s I
c-----------------------------------------------------------------------
c on entry:
...
c diagonal elements.
c
      ko = ia(nrow+1)+icount; print *, "a1", ko
c
c     copy rows backward
c
      do 5 ii=nrow, 1, -1
...
c
         ...
         do 4 k = k2,k1,-1
            ...
            if (test .and. (j .lt. ii)) then
               test = .false.
               ko = ko - 1
               a(ko) = scal; print *, ko, shape(a), shape(ja)
               ja(ko) = ii
               iw(ii) = ko
            endif
            ko = ko-1
...
c-----------------------------------------------------------------------
      subroutine apldia_cmplx(nrow, job, a, ja, ia, diag, b, jb, ib, iw)

      use precisions, only: cwp

      complex(cwp) a(*), b(*), diag(nrow)
      integer ja(*),jb(*), ia(nrow+1),ib(nrow+1), iw(*)
c-----------------------------------------------------------------------
c Adds a diagonal matrix to a general sparse matrix:  B = A + Diag
c-----------------------------------------------------------------------
