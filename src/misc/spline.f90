...
!	##############################################################################################################################
!
!SPLINE is a FORTRAN90 library which defines and evaluates spline functions.
!
...
!	##############################################################################################################################
!
!TOMS446 is a FORTRAN90 library which implements ACM TOMS algorithm 446, for the manipulation of Chebyshev series, by Roger Broucke.
!
...
subroutine cheby_zero ( a,b, n, x )
!*****************************************************************************80
!
!! CHEBY_ZERO returns zeroes of the Chebyshev grid: cos(pi*(0:N)/N)
!
!  Parameters:
!
...
!
  ...
  real ( kind = 8 ) angle, x(n)


  do j = 1, n
!    angle = real ( 2 * j - 1, kind = 8 ) * pi / real ( 2 * n, kind = 8 )
	angle = real ( 2 * ( n - j ) + 1, kind = 8 ) * pi / real ( 2 * n, kind = 8 )
    x(j) = cos ( angle )
  end do
...
subroutine chebyshev_coefficients ( n, fx, c )
...
!    Input, integer ( kind = 4 ) N, the order of the interpolant.
!
!    Input, real ( kind = 8 ),  FX, the values of the function at Chebyshev points in [a, b]
!
...
!
  ...
  do k = 1, n
    c(k) = 0.0D+00
    do j = 1, n
      angle = real ( ( k - 1 ) * ( 2 * j - 1 ), kind = 8 ) * pi &
        / real ( 2 * n, kind = 8 )
      c(k) = c(k) + fx(n-j+1) * cos ( angle ) ! values fx has to be flipped bc of the inverse ordering of chebyshev nodes implied by this procedure
    end do
  end do
...
subroutine chebyshev_interp ( a, b, n, c, m, x, cf )

!*****************************************************************************80
!
!! chebyshev_interp evaluates a Chebyshev interpolant.
!
!  Parameters:
!
!    Input, real ( kind = 8 ) A, B, the domain of definition.
...
subroutine chebyshev_deriv_interp ( a,b, npl, coef, m, x, dfx )

!*****************************************************************************80
!
!! chebyshev_deriv_interp evaluates the derivative of a Chebyshev series at a point.
!
!  Discussion:
!
...
do l = 1, m
  ...
  y = ( 2.0D+00 * x(l) - a  - b ) / ( b - a ) ! 2/(b-a) to adjust for [-1;1] -> [a;b]
  xjp2 = 0.0D+00
  xjpl = 0.0D+00
  bjp2 = 0.0D+00
  bjpl = 0.0D+00
  ...
  dfx(l) = ( bj - bf ) / ( b - a ) ! 2/(b-a) to adjust for [-1;1] -> [a;b]
...
subroutine chebyshev_quadr ( a, b, n, f, f_int )

!*****************************************************************************80
!
!! chebyshev_quadr approximates the integral over [a,b] using the given Chebyshev coefficients.
!
!  Parameters:
!
!    Input, real ( kind = 8 ) A, B, the domain of definition.
...
!    Input, real ( kind = 8 ) C(N), the Chebyshev coefficients.
!
!    Output, real ( kind = 8 ) F_INT, the value of the integral
!    polynomial at X.
!
  implicit none
  ...
  f_int = 0.0D+00
    do j = 1, n
      angle = real ( ( 2 * j - 1 ), kind = 8 ) * pi / real ( 2 * n, kind = 8 )
      x = cos ( angle )
      fx = ((b-a)/2)*f(((b-a)/2)*x + (b+a)/2) * (1-x**2)**.5
      f_int = f_int + w * fx ! values fx has to be flipped bc of the inverse ordering of chebyshev nodes implied by this procedure
    end do
